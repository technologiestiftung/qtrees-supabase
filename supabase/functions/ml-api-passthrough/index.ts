...
if (
  ml_pgrest_host === undefined || ml_pgrest_user === undefined ||
  ml_pgrest_password === undefined || ml_pgrest_port === undefined ||
...
const body_internal_server_error = JSON.stringify({ message: "internal server error" });
...
serve(async (req: Request) => {
  ...
  const target_url =
    `${ml_pgrest_host}:${ml_pgrest_port}${url.pathname}${url.search}`;
  ...
  if (url.pathname === "/") {
    const paths = Object.keys(json.paths).filter((p) =>
      p !== "/" && p !== "/rpc/login"
    );
